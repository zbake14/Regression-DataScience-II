...
object LassoRegression extends App
{
  ...
  def ForwardSelection(argX: MatrixD, argY: VectorD, datasetName:String): Unit = {
    ...
    //Create object of LassoRegression and train and evaluate the model.
    ...
    var fcols = Set[Int]() // start with no variable. Indices of columns.
    ...
    for (l <- 0 until x.dim2) {
      if (flag) {
        ...
        if (fit_j(7) < 0 || fit_j(0)<0) flag = false //break the loop if indices are negative to avoid run time errors.
        if(l<3) flag = true
        if(flag)
        {
